...
const UserDashboard = ({ setIsLoggedIn }) => {
    ...
    const { data: currentUser } = useQuery({
        ...
        onSuccess: (data) => {
            ...
            // Fetch notifications
        const { data: notificationsData, refetch, isLoading, error: notificationError } = useQuery({
            ...
            <header className="fixed top-0 left-0 right-0 bg-black px-4 sm:px-6 py-4 z-50">
                <div className="w-full flex items-center justify-between">
                    ...
                    <nav className="hidden md:flex items-center space-x-6 lg:space-x-8">
                        <Link to="/dashboard" className="text-cyan-400 font-medium hover:text-cyan-300 transition-colors">
                            Dashboard
                        </Link>
                        <Link to="/bookings" className="text-white/70 font-medium hover:text-white transition-colors">
                            My Bookings
                        </Link>
                        <Link to="/rooms" className="text-white/70 font-medium hover:text-white transition-colors">
                            Available Rooms
                        </Link>
                        <Link to="/profile" className="text-white/70 font-medium hover:text-white transition-colors">
                            Profile
                        </Link>
                            ...
                            {notificationsOpen && (
                                <div className="notifications-dropdown px-6 py-3 border-t border-gray-700">
                                    <div className="text-sm text-blue-200 mb-3">
                                        <p className="font-medium">Notifications</p>
                                        <p className="text-xs text-gray-400">{notifications.length} new</p>
                                    </div>
                                    {notifications.length > 0 ? (
